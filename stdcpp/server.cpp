...
void server::delConnect(int fd) {
  ...
  if (it != fdmap.end()) {
    ...
    if (con->clientfd == fd) con->clientfd = -1;
    if (con->serverfd == fd) con->serverfd = -1;
  ...
  if (len < 0) {
    ...
    if (buf[0] == 0x05) {
      if (buf[1] == 0x01)  // CMD为Connect
      {
        ...
        if (serverfd < 0 || connect(serverfd, (struct sockaddr*)&server_addr,
  ...
  while (1) {
    ...
    for (int i = 0; i < n; ++i) {
      if (events[i].data.fd == listenfd) {
        ...
        //已有连接
        threadpool.add_task(
            std::bind(&server::eventHandle, this, events[i].data.fd));
      }
    }
  }
}